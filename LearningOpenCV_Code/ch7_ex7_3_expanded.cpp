...
int main(int argc, char** argv) {
	...
	if (argc == 5) {
		...
		if (src[0]->widthStep != tmp->widthStep) {
			printf(
					"Error, Withstep of alloated src doesn't equal withStep of loaded image [%d vs %d]",
		...
		//
		IplImage *hsv[5], *h_plane[5], *s_plane[5], *v_plane[5],
				*planes[5][2];
		IplImage* hist_img[5];
		CvHistogram* hist[5];
		...
		for (i = 0; i < 5; ++i) {
			...
			cvCvtPixToPlane(hsv[i], h_plane[i], s_plane[i],
					v_plane[i], 0);
			// Build the histogram and compute its contents.
			//
			{
				hist[i] = cvCreateHist(2, hist_size,
			...
			for (int h = 0; h < h_bins; h++) {
				for (int s = 0; s < s_bins; s++) {
					float bin_val =
							cvQueryHistValue_2D( hist[i], h, s );
					int intensity =
							cvRound(
									bin_val
											* 255
		...
		printf(
				"Comparison                         Corr                    Chi                     Intersect               Bhat\n");
				...
				for (int s = 0; s < s_bins; s++) {
					...
					cvSet2D(
							...
							cvScalar(
									bin_val,
									bin_val,
									bin_val)); //Point weight
					cvSet2D(sig[i], h * s_bins + s, 1,
							cvScalar(h)); //Coord 1
					cvSet2D(sig[i], h * s_bins + s, 2,
							cvScalar(s)); //Coord 2
				}
			}
		}
