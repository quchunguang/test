...
int cvupdateCodeBook(uchar *p, codeBook &c, unsigned *cbBounds,
	...
	if (i == c.numEntries) //No existing code word found, make a new one
			{
		code_element **foo = new code_element*[c.numEntries + 1];for(int ii=0; ii<c.numEntries; ii++)
		{
			foo[ii] = c.cb[ii];
		}
...
// PAINT THE FOUND REGIONS BACK INTO THE IMAGE
	...
	if (num != NULL) {
		...
		for (i = 0, c = contours; c != NULL; c = c->h_next, i++) {
			if (i < N) //Only process up to *num of them
				...
				if (centers != NULL) {
					...
					M00 = cvGetSpatialMoment(&moments, 0,
							0);
					M10 = cvGetSpatialMoment(&moments, 1,
							0);
					M01 = cvGetSpatialMoment(&moments, 0,
							1);
			...
			cvDrawContours(mask, c, CV_CVX_WHITE, CV_CVX_WHITE, -1,
					CV_FILLED, 8); //draw to central mask
	...
	//ELSE JUST DRAW PROCESSED CONTOURS INTO THE MASK
		...
		for (c = contours; c != NULL; c = c->h_next) {
			cvDrawContours(mask, c, CV_CVX_WHITE, CV_CVX_BLACK, -1,
					CV_FILLED, 8);
		}
	}
}
