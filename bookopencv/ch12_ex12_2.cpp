	...
	while (successes < n_boards) {
		...
		if ((frame++ % board_dt) == 0) {
			...
			if (corner_count == board_n) {
				...
				successes++;
				printf(
						"Collected our %d of %d needed chessboard images\n",
						successes, n_boards);
			} else
...
// create pointers to 2D points in points1, points2, initialize it etc...
	...
	CvPoint2D32f* imgpt2 = (CvPoint2D32f*) (image_points2->data.fl
			+ point_count);
	...
	int fm_count = cvFindFundamentalMat(points1, points2,
			fundamental_matrix, CV_FM_RANSAC, 1.0, 0.99, status);
	...
	for (int r = 0; r < 3; r++) {
		for (int c = 0; c < 3; c++) {
			printf(
					"%+10f  ",
					CV_MAT_ELEM( *fundamental_matrix, float, r, c));
		}
		printf("\n");
	}
	printf("\n");
