...
int build_rtrees_classifier(char* data_filename, char* filename_to_save,
	...
	if (filename_to_load) {
		...
		sample_idx = cvCreateMat(1, nsamples_all, CV_8UC1);
		{
			...
			cvGetCols(sample_idx, &mat, ntrain_samples,
					nsamples_all);
			cvSetZero(&mat);
		}
		...
		forest.train(
				...
				var_type,
				0,
				CvRTParams(10, 10, 0, false, 15, 0, true, 4,
						100, 0.01f, CV_TERMCRIT_ITER));
		printf("\n");
	}
	...
	for (i = 0; i < nsamples_all; i++) {
		...
		r = fabs((double) r - responses->data.fl[i]) <= FLT_EPSILON ?
				1 : 0;
	...
	if (var_importance) {
		...
		for (i = 0; i < var_importance->cols; i++)
			printf(
					"%-2d\t%-4.1f\n",
					i,
					100.f * var_importance->data.fl[i]
							/ rt_imp_sum);
	}
			...
			"Proximities between some samples corresponding to the letter 'T':\n");
	{
		...
		const int pairs[][2] = { { 0, 103 }, { 0, 106 }, { 106, 103 }, {
				-1, -1 } };
		...
		for (i = 0; pairs[i][0] >= 0; i++) {
			...
			printf(
					...
					pairs[i][0],
					pairs[i][1],
					forest.get_proximity(&sample1, &sample2)
							* 100.);
		}
	}
...
static
int build_boost_classifier(char* data_filename, char* filename_to_save,
		char* filename_to_load) {
	...
	if (filename_to_load) {
		...
		CvMat* new_data = cvCreateMat(ntrain_samples * class_count,
				var_count + 1, CV_32F);
		CvMat* new_responses = cvCreateMat(ntrain_samples * class_count,
				1, CV_32S);

		// 1. unroll the database type mask
		printf("Unrolling the database...\n");
		for (i = 0; i < ntrain_samples; i++) {
			float* data_row = (float*) (data->data.ptr
					+ data->step * i);
			for (j = 0; j < class_count; j++) {
				float* new_data_row =
						(float*) (new_data->data.ptr
								+ new_data->step
										* (i
												* class_count
												+ j));
				...
				new_responses->data.i[i * class_count + j] =
						responses->data.fl[i]
								== j + 'A';
			}
		}
		...
		boost.train(
				...
				var_type,
				0,
				CvBoostParams(CvBoost::REAL, 100, 0.95, 5,
						false, 0));
	...
	weak_responses = cvCreateMat(1, boost.get_weak_predictors()->total,
			CV_32F);
	...
	for (i = 0; i < nsamples_all; i++) {
		...
		r = fabs(best_class - responses->data.fl[i]) < FLT_EPSILON ?
				1 : 0;
...
static
int build_mlp_classifier(char* data_filename, char* filename_to_save,
		char* filename_to_load) {
	const int class_count = 26;
	CvMat* data = 0;
	CvMat train_data;
	...
	if (filename_to_load) {
		...
		for (i = 0; i < ntrain_samples; i++) {
			...
			float* bit_vec = (float*) (new_responses->data.ptr
					+ i * new_responses->step);
			for (j = 0; j < class_count; j++)
				bit_vec[j] = 0.f;
		...
		CvMat layer_sizes =
				cvMat(
						1,
						(int) (sizeof(layer_sz)
								/ sizeof(layer_sz[0])), CV_32S, layer_sz )
						;
	...
	if (i < argc
			|| (method == 0 ?
					build_rtrees_classifier(data_filename,
							filename_to_save,
							filename_to_load) :
				method == 1 ? build_boost_classifier(
						data_filename, filename_to_save,
						filename_to_load) :
				method == 2 ? build_mlp_classifier(
								data_filename,
								filename_to_save,
								filename_to_load) :
						-1) < 0) {
