...
int main(int argc, char* argv[]) {
	...
	int Nc = cvFindContours(img_edge, storage, &first_contour,
			sizeof(CvContour), CV_RETR_LIST // Try all four values and see what happens
			);
	int n = 0, k;
	...
	for (CvSeq* c = first_contour; c != NULL; c = c->h_next) {
		...
		cvDrawContours(img_8uc3, c, CVX_RED, //Yarg, these are defined above, but not in the book.  Oops
				CVX_BLUE, 0, // Try different values of max_level, and see what happens
				2, 8);
		printf("Contour #%d\n", n);
